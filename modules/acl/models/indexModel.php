...
class indexModel extends Model
{
    ...
    //Util_Permiso Jhon Martinez
    public function eliminarHabilitarPermiso($Per_IdPermiso = 0,$Row_Eliminar = 0)
    {
        try{

            $permiso = $this->_db->query(
                " UPDATE permisos SET Row_Eliminar = $Row_Eliminar WHERE Per_IdPermiso = $Per_IdPermiso "               
                );
            return $permiso->rowCount(PDO::FETCH_ASSOC);
        } catch (PDOException $exception) {
    ...
    //Util_Rol Jhon Martinez
    public function eliminarHabilitarRol($iRol_IdRol = 0, $iRow_Eliminar = 0)
    {
        try{
            $sql = "call s_u_habilitar_deshabilitar_rol(?,?)";
            $result = $this->_db->prepare($sql);
            $result->bindParam(1, $iRol_IdRol, PDO::PARAM_INT);
            $result->bindParam(2, $iRow_Eliminar, PDO::PARAM_INT);
            $result->execute();
