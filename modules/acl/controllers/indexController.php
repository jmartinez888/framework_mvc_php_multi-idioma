...
class indexController extends aclController
{
    ...
    public function roles()
    {
        ...
        $condicion = " ORDER BY r.Row_Eliminar DESC ";
        $soloActivos = 0;
        if (!$this->_acl->permiso('ver_eliminados')) {
            $soloActivos = 1;
            $condicion = " WHERE r.Row_Eliminar = $soloActivos ";
        }
        //Filtro por Activos/Eliminados
    ...
    public function _cambiarEstadoRol()
    {
        ...
        if ($txtBuscar) 
        {
            ...
            if (!$this->_acl->permiso('ver_eliminados')) {
                $soloActivos = 1;
                $condicion .= " AND r.Row_Eliminar = $soloActivos ";
            }
    ...
    public function _eliminarRol()
    {
        ...
        $Rol_IdRol = $this->getInt('_Rol_IdRol');
        $Row_Eliminar = $this->getInt('_Row_Eliminar');
        $txtBuscar = $this->getSql('palabra');
        $pagina = $this->getInt('pagina');
        $filas=$this->getInt('filas');       
        // echo $Per_IdPermiso."//".$Row_Eliminar;
        //Para Mensajes
        $resultado = array();
        $mensaje = "error";
        $contenido = "";
        ...
        if ($Row_Eliminar == 0) {
            ...
            {            
                ...
                if ($usu <= 0)
                {                   
                    $rowCount = $this->_aclm->eliminarHabilitarRol($Rol_IdRol,$Row_Eliminar);
                    // echo $rowCount3;//exit;
    ...
    public function permisos($error = "")
    {
        ...
        //Filtro por Activos/Eliminados
        $condicion = " ORDER BY p.Row_Eliminar DESC ";
        $soloActivos = 0;
        if (!$this->_acl->permiso('ver_eliminados')) {
            $soloActivos = 1;
            $condicion = " WHERE p.Row_Eliminar = $soloActivos ";
        }
        //Filtro por Activos/Eliminados
    ...
    public function _cambiarEstadoPermisos(){
        ...
        if ($txtBuscar) 
        {
            ...
            if (!$this->_acl->permiso('ver_eliminados')) {
                $soloActivos = 1;
                $condicion .= " AND p.Row_Eliminar = $soloActivos ";
            }
            $condicion .= " ORDER BY p.Row_Eliminar DESC  ";
        } else {
            //Filtro por Activos/Eliminados     
        ...
        if ($Row_Eliminar == 0) {
            ...
            {            
                ...
                if (!$role)
                {
                    ...
                    if(!$usuario){
                        $rowCount = $this->_aclm->eliminarHabilitarPermiso($Per_IdPermiso,$Row_Eliminar);
                        if($rowCount)
                        {
