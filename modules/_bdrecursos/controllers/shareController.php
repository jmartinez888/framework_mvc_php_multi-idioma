...
class shareController extends bdrecursosController
{
	...
	public function json($arg1 = '', $arg2 = '', $arg3 = '')
	{
		...
		if($arg1!='')
        {
		    ...
		    else 
		    {
        		...
        		$registros = $this->_bdrecursos->getRegistrosXRecurso($arg1, $id_recurso);
		    	...
		    	switch (str_replace(' ', '', strtolower($recursos[0]['Esr_Nombre']))) 
		        {
		            ...
		            default:
		                ...
		                foreach ($registros as $registro) 
				        {
				        	...
				        	$lista = $this->_registros->getListaRegistrosEstandar($fichaEstandar[0]['Fie_NombreTabla'], 'es', "AND $idTabla=".$registro[$idTabla]."");
	...
	public function excel($nombre_tabla='', $subtema='') 
	{
		$this->_acl->autenticado();
		$this->validarUrlIdioma();
		$this->_view->setTemplate(LAYOUT_FRONTEND);
        ...
        $variable = $this->_share->getTablaVariable($nombre_tabla, "WHERE $campo_subtema='".$subtema."'"); 
        ...
        for ($i = 0; $i < count($variable); $i++) 
        {
            ...
            $data = $this->_share->getTablaData($nombre_tabla, $variable[$i][0], $ficha[0]['Fie_ColumnaTabla']);

            $c=2;
            for ($j = 1; $j < count($data); $j++) 
            {      
           		$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow($i, $c, $data[$j][3]);
            	$c++;            	
            }
