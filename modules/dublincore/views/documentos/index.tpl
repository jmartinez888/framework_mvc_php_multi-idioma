        ...
        <div class="panel-body">
            <div class="row">
              ...
              <div class="col-md-6 col-md-offset-3 div-filtro text-center">
                ...
                <a class="badge" href="{$_layoutParams.root_clear}dublincore/documentos/buscarporpalabras/{$palabrabuscada|default:'all'}/all/{$filtroTipo|default:'all'}/{$filtroAutor|default:'all'}/{$filtroPais|default:'all'}">
                 Tema: {$filtroTema}  <i class="fa fa-times"></i> 
                </a>
                {/if}
                {if isset($filtroTipo)}
                <input type="hidden" id= "filtrotipodocumento" value="{$filtroTipo}">
                <a class="badge" href="{$_layoutParams.root_clear}dublincore/documentos/buscarporpalabras/{$palabrabuscada|default:'all'}/{$filtroTema|default:'all'}/all/{$filtroAutor|default:'all'}/{$filtroPais|default:'all'}">
                 Tipo: {$filtroTipo} <i class="fa fa-times"></i>               
                </a>
                {/if}
                {if isset($filtroAutor)}
                <input type="hidden" id= "filtroautordocumento" value="{$filtroAutor}">
                <a class="badge" href="{$_layoutParams.root_clear}dublincore/documentos/buscarporpalabras/{$palabrabuscada|default:'all'}/{$filtroTema|default:'all'}/{$filtroTipo|default:'all'}/all/{$filtroPais|default:'all'}">
                 Autor: {$filtroAutor} <i class="fa fa-times"></i>               
                </a>
                {/if}
                {if isset($filtroPais)}
                <input type="hidden" id= "filtropaisdocumento" value="{$filtroPais}">
                <a class="badge" href="{$_layoutParams.root_clear}dublincore/documentos/buscarporpalabras/{$palabrabuscada|default:'all'}/{$filtroTema|default:'all'}/{$filtroTipo|default:'all'}/{$filtroAutor|default:'all'}/all">
    ...
    <div id="paginar">
        <div class="table-responsive">
            <table class="table table-hover table-condensed">
              <thead>
                <tr><th>#<th>{$lenguaje["tabla_campo_titulo"]}<th>{$lenguaje["tabla_campo_descripcion_documentos"]}<th>{$lenguaje["tabla_campo_tipo_documento_documentos"]}<th>{$lenguaje["fechadocumentos_metadata_documentos"]}<th>{$lenguaje["tabla_campo_tipo_documentos"]}<th>
                {$lenguaje["tabla_campo_pais_documentos"]}<th>{$lenguaje["tabla_campo_enlaces_documentos"]}
                <tbody>
                   {if isset($documentos) && count($documentos)}
                            
                      {foreach item=datos from=$documentos}
                        <tr>
                            <td>{$numeropagina++}<td>{$datos.Dub_Titulo}<td>{$datos.Aut_Nombre}<br/><div data-toggle="tooltip" data-placement="right" title="{$datos.Dub_Descripcion}">{$datos.Dub_Descripcion|truncate:150:" ..."}</div>
                             ...
                             <td> <a class="btn btn-default" data-toggle="tooltip" data-placement="top" href="{$_layoutParams.root_clear}dublincore/documentos/metadata/{$datos.Dub_IdDublinCore}" target="_blank" title="{$lenguaje["tabla_campo_detalle_documentos"]}"><i class="glyphicon glyphicon-list-alt" ></i></a>
                            <a data-toggle="tooltip" data-placement="top" class="btn btn-default" href="{$_layoutParams.root_clear}dublincore/documentos/descargar/{$datos.Arf_PosicionFisica}/{$datos.Dub_IdDublinCore}" target="_blank" title="{$lenguaje["icono_descargar_documentos"]}"><i class="glyphicon glyphicon-download-alt" ></i></a> 
                      {/foreach}                       
